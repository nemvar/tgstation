/mutable_appearance/photo
	var/_step_x = 0
	var/_step_y = 0

/mutable_appearance/photo/New(location, atom/A)			//Intentionally not Initialize(), to make sure the clone assumes the intended appearance in time for the camera getFlatIcon.
	. = ..()
	if(istype(A))
		loc = location
...
/obj/item/camera/proc/camera_get_icon(list/turfs, turf/center, psize_x = 96, psize_y = 96, datum/turf_reservation/clone_area, size_x, size_y, total_x, total_y)
	...
	if(istype(clone_area) && total_x == clone_area.width && total_y == clone_area.height && size_x >= 0 && size_y > 0)
		...
		for(var/t in turfs)
			...
			if(!(newT in clone_area.reserved_turfs))		//sanity check so we don't overwrite other areas somehow
				continue
			images += new /mutable_appearance/photo(newT, T)
			if(T.loc.icon_state)
				images += new /mutable_appearance/photo(newT, T.loc)
			for(var/i in T.contents)
				var/atom/A = i
				if(!A.invisibility || (see_ghosts && isobserver(A)))
					images += new /mutable_appearance/photo(newT, A)
		skip_normal = TRUE
		wipe_images = TRUE
	...
	if(!skip_normal)
		for(var/i in turfs)
			var/turf/T = i
			images += new /mutable_appearance/photo(T.loc, T)
			for(var/atom/movable/A in T)
				if(A.invisibility)
					if(!(see_ghosts && isobserver(A)))
						continue
				images += new /mutable_appearance/photo(A.loc, A)
			CHECK_TICK
	...
	for(var/i in 1 to images.len)
		var/mutable_appearance/c = images[i]
	...
	for(var/Adummy in sorted)
		var/mutable_appearance/photo/A = Adummy
